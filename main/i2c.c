...
void lcd_send_cmd (char cmd)
{
	...
	if (i2c_master_write_to_device(I2C_NUM, SLAVE_ADDRESS_LCD, data_t, 4, 1000)!=0) ESP_LOGW(TAG, "Error in sending command");
	usleep(100);
}
...
void lcd_send_data (char data)
{
	...
	if (i2c_master_write_to_device(I2C_NUM, SLAVE_ADDRESS_LCD, data_t, 4, 1000)!=0) ESP_LOGW(TAG, "Error in sending data");
	usleep(100);
}
